...
from .rotation_conversions import quaternion_to_matrix, Lie
...
class PoseNet(torch.nn.Module):
    ...
    def forward(self, time):
        ...
        rotmat_est = quaternion_to_matrix(rots_feat_est)
        # make c2w
    ...
    def forward(self, cfg, index):
        ...
        for li,layer in enumerate(self.mlp_quad):
            ...
            if li==len(self.mlp_quad)-1:
                if cfg['end_activ']:
                    rotation_feat[:,1:] =  torch_F.tanh(rotation_feat[:,1:])
                    rotation_feat[:,0] =  1 - torch_F.tanh(rotation_feat[:,0])
                else:
                    rotation_feat[:,0] = 1 - rotation_feat[:,0]
            else:
